...
try{
	...
	if(rs.next()){
		...
		String pass=decrypt(str);
		if(pass.equals(password)){
			session.setAttribute("logged_user", username);
			session.setAttribute("is_logged", "yes");
			RequestDispatcher rd=request.getRequestDispatcher("profile.jsp?value=0");
			rd.forward(request,response);
		}else{
			RequestDispatcher rd=request.getRequestDispatcher("login.jsp?value=98");
			rd.forward(request,response);
		}
	}else{
		RequestDispatcher rd=request.getRequestDispatcher("login.jsp?value=98");
